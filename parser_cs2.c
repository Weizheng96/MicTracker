...
FILE *fp100;
// fp100 = fopen("sample.inp","r");
fp100 = fopen("myCs2Imput.txt","r");

   
while ( fgets ( in_line , 1024, fp100) != NULL )
  {
  no_lines ++;
  // printf("%d : %s\n", no_lines,in_line );
  ...
  switch (in_line[0])
    {
      ...
      case 'p':                  /* problem description      */
		...
		for ( ndp = nodes; ndp < nodes + n; ndp ++ )
         ...
         for (ndp = nodes, ii = 0; ndp < nodes + n; ndp ++, ii ++ )
             ndp -> id = ii;
...
//  { err_no=EN21; printf("reading error\n"); goto error; } 

if ( no_lines == 0 ) /* empty input */
  { err_no = EN22; printf("empty input\n");goto error; } 

if ( no_alines < m ) /* not enough arcs */
  { err_no = EN19; printf("not enough arcs \n");goto error; } 

if ( ABS( total_p - total_n ) > 0.5 ) /* unbalanced problem */
  { err_no = EN13; printf("unbalanced problem\n");goto error; }
